...
class MainController extends Controller
{
    ...
    public function getTopHotelsByFavorites(Request $request)
    {
        try {
            $take = $request->query('take');
            $skip = $request->query('skip');
    ...
    public function getTopHotelsByReviews(Request $request)
    {
        try {
            $take = $request->query('take');
            $skip = $request->query('skip');
    ...
    public function getRoomsByHotelCode(string $id)
    {
        try {
            $result = Room_types::select('room_types.*')
                ...
                ->selectRaw('(SELECT COUNT(rooms.code) FROM rooms WHERE rooms.room_types_code = room_types.code AND rooms.hotel_code = ?) as room_count', [$id])
                ->selectRaw('(SELECT COUNT(rooms.code) FROM rooms WHERE rooms.room_types_code = room_types.code AND rooms.hotel_code = ? AND rooms.status = 0) as room_available', [$id])
                ->selectRaw('(CASE WHEN (SELECT COUNT(rooms.code) FROM rooms WHERE rooms.room_types_code = room_types.code AND rooms.hotel_code = ? AND rooms.status = 0) > 0 THEN true ELSE false END) as isAvailable', [$id])
                ->selectRaw('(SELECT MIN(rooms.price) FROM rooms WHERE rooms.room_types_code = room_types.code AND rooms.hotel_code = ? AND rooms.status = 0) as lowest_price', [$id])
